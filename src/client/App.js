...
export default class App extends Component {
    ...
    handleTestCsv = (event) => {
        ...
        fetch(`/api/run_on_csv?name=${encodeURIComponent(this.state.name)}`)
    ...
    handleTestFileDir = (event) => {
        event.preventDefault();
        fetch(`/api/create_dir?rows=${this.state.rows}&columns=${this.state.cols}`);
    };

    handleVideoScraping = (event) => {
        event.preventDefault();
        fetch(`/api/cut_video`);
    };
        ...
        return (
            <div className="App">
                <header className="App-header">
                    ...
                    <form onSubmit={this.handleVideoScraping}>
